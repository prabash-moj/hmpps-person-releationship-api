...
class CountryService(private val countryRepository: CountryRepository) {
  ...
  fun getCountryById(id: Long): Country = countryRepository.findByCountryId(id)
    .orElseThrow { EntityNotFoundException("Country with id $id not found") }.toModel()

  @Transactional(readOnly = true)
  fun getCountryByNomisCode(code: String): Country = countryRepository.findByNomisCode(code)
    .orElseThrow { EntityNotFoundException("Country with nomis code $code not found") }.toModel()

  @Transactional(readOnly = true)
  fun getCountryByIsoAlpha2(code: String): Country = countryRepository.findByIsoAlpha2(code)
    .orElseThrow { EntityNotFoundException("Country with iso alpha code 2 $code not found") }.toModel()

  @Transactional(readOnly = true)
  fun getCountryByIsoAlpha3(code: String): Country = countryRepository.findByIsoAlpha3(code)
